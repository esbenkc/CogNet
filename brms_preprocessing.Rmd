...
lost_dat <- week_friends %>% 
  mutate(lost_con = friendship < 0.5 * previous_weight, 
         lockdown = week %within% first_lockdown | week %within% third_sem,
         timecol = as.factor(as.integer(as.factor(week)))) %>% 
  select(pair_key, timecol, friendship, friendship_strength = previous_weight, lost_con, lockdown, week)  
...
full_dat %>% 
  ...
  mutate(friendship = replace_na(friendship, 0),
         previous_weight = rollsum(friendship, 4, fill = NA, align = "right") / 4,
         lost_con = (friendship < 0.5 * previous_weight) & previous_weight > 0.001, 
         lockdown = week %within% first_lockdown | week %within% third_sem) %>% 
  drop_na() %>%  
