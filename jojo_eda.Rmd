...
add_connection_logic <- function(friendship_df, window_size=7) {
  friendship_df %>% 
      mutate(
    previous_streak = rollsum(is_friend, k=window_size, fill=NA, align="right"),
    next_streak = rollsum(is_friend, k=window_size, fill=NA, align="left"),
...
test_cluster <- kmeans(plot_data$log_msgs, centers=2)

get_clusters <- function(x, num_clusters=2) {
  as.factor(kmeans(x, centers=num_clusters)$cluster)
}

clusters <- get_clusters(plot_data$log_msgs)
