...
# chain plot 
```{r}
bayes_pos <- as.array(brm_result$fit)
...
```
```{r}
posterior_comparison <- function(posterior, variable = ":trans") {
  pos_col <- paste0("b_lockdownTRUE", variable)
  neg_col <- pasteo("b_lockdownFALSE", variable)
  posterior %>% 
    mutate(pos_df = !!sym(pos_col)-!!sym(neg_col)) %>% 
    pull(pos_df) %>% 
    density()
}
 
length(density(c(1,2,1,1))$y)
```
```{r}
library(dagitty)
lockdown_dag <- dagitty("dag {
  lockdown -> pagerank -> new_cons 
  time ->  new_cons
  lockdown -> clustering -> new_cons
  lockdown -> new_cons
        }
")

impliedConditionalIndependencies(lockdown_dag)
dagitty::adjustmentSets(lockdown_dag, exposure = "clustering", outcome = "new_cons")
